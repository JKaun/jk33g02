...
let volume = 1
window.onkeydown = teclav;

function teclav(event) {
	let nu = event.keyCode;
...
const MENU_MUSIC = Symbol('MENU_MUSIC');


//More music controls

function changeSong() {
	if (musicaFacha.played) {
		musicaFacha.pause;
		musicaSins.play;
	}
	else if (musicaSins.played) {
		musicaSins.pause;
		musicaFacha.play;
	}
}

window.onkeydown = teclaC;
function teclaC(change) {
	let nu = change.keyCode;
...
function renderScoreHud() {
	if (isCasualGame()) {
		...
		scoreNode.innerText = `PUNTAJE: ${state.game.score}`;
